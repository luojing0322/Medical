 ...
 * Date: 16/3/7 下午2:32.
 */
public class KMeans2d extends BaseProcessor {

    private String baseFile = docPath + "/immune/immune.xls";
    private String trainFile = docPath + "/data/wine.data";
     ...
     * @return
     */
    private List<ArrayList<Double>> readFacList() {
     ...
     * @return
     */
    private List<ArrayList<Double>> getDataList() {
        //读入原始数据
    ...
    public static void main(String[] args) throws IOException {
        ...
        KMeans2d kMeans2d = new KMeans2d();
        ...
        List<ArrayList<Double>> dataList = kMeans2d.getDataList();
...
//
        ...
        while (true) {//进行若干次迭代，直到聚类中心稳定
            ...
            for (int i = 0, datasize = dataList.size(); i < datasize; i++) {//标注每一条记录所属于的中心
                double minDistance = 99999999;
                int centerIndex = -1;
                for (int j = 0; j < k; j++) {//离0~k之间哪个中心最近
                    double currentDistance = 0;
                    for (int t = 1, size = centers.get(0).size(); t < size; t++) {//计算两点之间的欧式距离
            ...
            for (int i = 0; i < k; i++) {
                ArrayList<Double> tmp = new ArrayList<Double>();
                for (int j = 0, csize = centers.get(0).size(); j < csize; j++) {
                    double sum = 0;
                    for (int t = 0, hsize = helpCenterList.get(i).size(); t < hsize; t++)
                        sum += helpCenterList.get(i).get(t).get(j);
                    tmp.add(sum / helpCenterList.get(i).size());
                }
                newCenters.add(tmp);
            ...
            for (int i = 0; i < k; i++) {
                for (int j = 1, size = centers.get(0).size(); j < size; j++) {//计算两点之间的欧式距离
